...
public class MyUtils {
	...
	public int sum(List<Integer> list) { // reçoit une liste déjà créée que je nomme ici list
		int res = 0;
		for (int i = 0; i < list.size(); i++) {
			// res = res + list.get(i); // un tableau aurait été écrit comme "list[i]"
			res += list.get(i); // += permet d'ajouter la valeur courante + celle qu'on lui donne
		}
	...
	public String emptyToNull(String s) {
		...
		if (s == null || s.isEmpty()) // trim().equals("") ) // subtring(0).equals(""))retire non pas des lettres
										// comme explique la definition officielle, ici substring(0) retire les espaces
										// enregistrés
		{
			return null;
		}
	...
	public List<Integer> filterEvenNumbers(List<Integer> integers) {
		...
		List<Integer> res = new ArrayList<Integer>(); // mes résultats comprennent une Liste de nomhres entiers; elle se
														// représente par une liste
	...
	public List<Float> transformSquarePlusPointFive(List<Integer> integers) {
		...
		for (int i = 0; i < integers.size(); i++) {
			...
			// le (float) est ajouté car le calcul ne peut se faire entre un format int et
			// format float
	...
	public List<Integer> Puissance2(List<Integer> integers) {
